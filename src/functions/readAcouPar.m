...
% It reads acouPar txt files in given a folder and generates a unique table
arguments
    dataFolder string
    options.type
end
...
for iFile = 1:length(txtFiles)
    ...
    acquisitionId = extractBefore(cellstr(filename),".txt");
    ...
    parTb = transformTable(rawData,parTb,acquisitionId,type=options.type);
end
